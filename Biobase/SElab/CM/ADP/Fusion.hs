...
import           Data.PrimitiveArray hiding (map, unsafeIndex)  -- TODO disable after bug-free (remove .Checked)
...
instance
  ...
  termStream (ts:|EmitChar xs) (cs:._) (us:.u) (is:.ix)
    ...
    where mk tstate = return (tstate :. VU.length xs -1)
...
instance
  ...
  termStream (ts:|Terminally) (cs:.IStatic ()) (us:._) (is:.ix@(StateIx styC styA i))
    ...
    where go (TState s ii ee) =
            ...
            in  TState s (ii:.:RiSixI chd (-1)) (ee:.StateIx styC styA chd)
          {-# Inline [0] go #-}
